...
class AliPay(object):
    ...
    def direct_settle(self, out_request_no, trade_no,trans_out,trans_in,amount, **kwargs):
        """
        结算
        ...
        :return:
        """
        a=[{"trans_out":trans_out,"trans_in":trans_in,"amount":amount,"amount_percentage":100}]

        biz_content = {
            "out_request_no":out_request_no,
            "trade_no": trade_no,
            "royalty_parameters":a,
            "operator_id":"A0001"
        }
        biz_content.update(kwargs)
        data = self.build_body1("alipay.trade.order.settle", biz_content)
        return self.sign_data(data)

    def direct_refund(self, trade_no, amount, **kwargs):
            """
            退款
            :param self:
            ...
            biz_content.update(kwargs)
            data = self.build_body1("alipay.trade.refund", biz_content, self.return_url)
            return self.sign_data(data)
    ...
    def build_body1(self, method, biz_content):
        data = {
            "app_id": self.appid,
            "method": method,
            "charset": "utf-8",
